...
const rutasDocentes = (0, express_1.Router)();
rutasDocentes.get("/estudiantes", async (req, res) => {
    ...
    `;
        ...
        const rows = await (0, bd_1.consultar)(sql, params);
        res.json(rows);
    }
    catch (error) {
        const err = error;
...
rutasDocentes.get("/estudiantes/:id/resumen", async (req, res) => {
    try {
        ...
        const resumen = await (0, bd_1.consultar)(`
      SELECT
      ...
      WHERE id_estudiante = ?
    `, [idEst]);
        const juegos = await (0, bd_1.consultar)(`
    ...
    `);
        ...
        const mejores = await (0, bd_1.consultar)(`
      SELECT
        s.id_juego,
